    ...
    <div id="app">
        <nav class="flex items-center justify-between flex-wrap bg-header section p-6 py-4">
            ...
            <div class="w-full block lg:flex lg:items-center lg:w-auto">
                ...
                <dropdown width="150px" align="right">
                    <template v-slot:trigger>
                        <button class="flex items-center text-default no-underline text-sm">
                            <img src="{{ gravatar_url(Auth::user()->email) }}" alt="{{ Auth::user()->name }}'s avatar"
                                class="rounded-full mr-3 w-8">
                            {{ Auth::user()->name }}
                        </button>
                    </template>
                    <a href="{{ route('logout') }}"
                        class="button inline-block text-sm px-4 py-2 leading-none border rounded text-default border-white hover:border-transparent hover:text-teal-500 hover:bg-white lg:mt-0"
